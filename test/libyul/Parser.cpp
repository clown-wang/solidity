...
{
shared_ptr<Block>
parse(string const& _source, Dialect const& _dialect, ErrorReporter& errorReporter)
{
	try
	{
		...
		if (parserResult)
		{
			...
			if (yul::AsmAnalyzer(analysisInfo, errorReporter, _dialect).analyze(*parserResult))
				return parserResult;
		}
	}
...
std::optional<Error>
parseAndReturnFirstError(string const& _source, Dialect const& _dialect, bool _allowWarnings = true)
{
	ErrorList errors;
...
bool successParse(
	...
	Dialect const& _dialect = Dialect::yulDeprecated(),
...
#define CHECK_ERROR(text, typ, substring) \
	CHECK_ERROR_DIALECT(text, typ, substring, Dialect::yulDeprecated())

BOOST_AUTO_TEST_SUITE(YulParser)

BOOST_AUTO_TEST_CASE(smoke_test) { BOOST_CHECK(successParse("{ }")); }

BOOST_AUTO_TEST_CASE(vardecl) { BOOST_CHECK(successParse("{ let x:u256 := 7:u256 }")); }
...
BOOST_AUTO_TEST_CASE(vardecl_empty) { BOOST_CHECK(successParse("{ let x:u256 }")); }
...
{
	BOOST_CHECK(
		successParse("{ function add(a:u256, b:u256) -> c:u256 {} let y:u256 := 2:u256 let x:u256 "
					 ":= add(7:u256, add(6:u256, y)) }")
	);
}
...
{
	BOOST_CHECK(
		successParse("{ let x:u256 := 7:u256 { let y:u256 := 3:u256 } { let z:u256 := 2:u256 } }")
	);
}
...
BOOST_AUTO_TEST_CASE(function_definitions_multiple_args)
{
	BOOST_CHECK(successParse(
		"{ function f(a:u256, d:u256) { } function g(a:u256, d:u256) -> x:u256, y:u256 { } }"
	));
}
...
{
	BOOST_CHECK(
		successParse("{ function f(a:u256) -> b:u256 {} function g(a:u256, b:u256, c:u256) {} "
					 "function x() { g(1:u256, 2:u256, f(3:u256)) x() } }")
	);
}
...
{
	BOOST_CHECK(successParse(
		"{ function f() -> a:u256, b:u256, c:u256 {} let x:u256, y:u256, z:u256 := f() }"
	));
}
...
BOOST_AUTO_TEST_CASE(for_statement)
{
	...
	BOOST_CHECK(
		successParse("{ for {let i := 0} iszero(eq(i, 10)) {i := add(i, 1)} {} }", dialect)
	);
}
...
BOOST_AUTO_TEST_CASE(for_statement_break)
{
	auto const& dialect = EVMDialect::strictAssemblyForEVMObjects(EVMVersion::constantinople());
	BOOST_CHECK(
		successParse("{ for {let i := 0} iszero(eq(i, 10)) {i := add(i, 1)} {break} }", dialect)
	);
}
...
BOOST_AUTO_TEST_CASE(for_statement_continue)
{
	auto const& dialect = EVMDialect::strictAssemblyForEVMObjects(EVMVersion::constantinople());
	BOOST_CHECK(
		successParse("{ for {let i := 0} iszero(eq(i, 10)) {i := add(i, 1)} {continue} }", dialect)
	);
}
...
BOOST_AUTO_TEST_CASE(if_statement_invalid)
{
	...
	CHECK_ERROR(
		...
		ParserError,
		"Expected '{' but got reserved keyword 'let'"
	);
...
{
	CHECK_ERROR(
		"{ switch 0:u256 case 0:u256 {} case 0x0:u256 {} }",
		DeclarationError,
...
{
	CHECK_ERROR(
		"{ switch 0:u256 case 0:u256 {} case \"\":u256 {} }",
		DeclarationError,
		"Duplicate case defined."
	);
...
{
	BOOST_CHECK(
		successParse("{let x:u256 switch x case \"01234567890123456789012345678901\":u256 {}}")
	);
	CHECK_ERROR(
		"{let x:u256 switch x case \"012345678901234567890123456789012\":u256 {}}",
		TypeError,
		"String literal too long (33 > 32)"
	);
}
...
BOOST_AUTO_TEST_CASE(builtins_parser)
{
	...
	CHECK_ERROR_DIALECT(
		"{ let builtin := 6 }",
		ParserError,
		"Cannot use builtin function name \"builtin\" as identifier name.",
		dialect
	);
	CHECK_ERROR_DIALECT(
		"{ function builtin() {} }",
...
BOOST_AUTO_TEST_CASE(builtins_analysis)
{
	...
	CHECK_ERROR_DIALECT(
		"{ let a, b := builtin(1, 2) }",
		DeclarationError,
		"Variable count mismatch: 2 variables and 3 values.",
		dialect
	);
}
...
BOOST_AUTO_TEST_CASE(default_types_set)
{
	...
	// should be omitted.
	BOOST_CHECK_EQUAL(
		AsmPrinter{EVMDialectTyped::instance(EVMVersion{})}(*result),
		"{\n"
		"    let x:bool := true\n"
		"    let z:bool := true\n"
		"    let y := add(1, 2)\n"
