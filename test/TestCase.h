...
public:
	...
	/// Runs the test case.
	/// Outputs error messages to @arg _stream. Each line of output is prefixed with @arg
	/// _linePrefix. Optionally, color-coding can be enabled (if @arg _formatted is set to true).
	...
	/// Each line of output is prefixed with @arg _linePrefix.
	virtual void printUpdatedExpectations(std::ostream& _stream, std::string const& _linePrefix)
		const;
...
protected:
	...
	static void
	expect(std::string::iterator& _it, std::string::iterator _end, std::string::value_type _c);
	...
	TestCase::TestResult
	checkResult(std::ostream& _stream, const std::string& _linePrefix, bool const _formatted);
