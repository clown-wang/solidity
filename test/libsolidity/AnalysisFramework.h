...
{
protected:
	virtual std::pair<SourceUnit const*, langutil::ErrorList> parseAnalyseAndReturnError(
	...
	langutil::ErrorList
	expectError(std::string const& _source, bool _warning = false, bool _allowMultiple = false);
	...
	langutil::ErrorList filterErrors(langutil::ErrorList const& _errorList, bool _includeWarnings)
		const;
...
// Because of the limitations of the preprocessor, you cannot use {{T1, "abc"}, {T2, "def"}} as
// arguments, but have to replace them by (std::vector<std::pair<Error::Type, std::string>>{"abc",
// "def"}) (note the parentheses)
