...
string
ProtoConverter::getVarDecl(string const& _type, string const& _varName, string const& _qualifier)
{
...
template <typename T>
pair<string, string>
ProtoConverter::assignChecker(string const& _varName, string const& _paramName, T _type)
{
	ostringstream local;
	AssignCheckVisitor
...
std::string
ValueGetterVisitor::croppedString(unsigned _numBytes, unsigned _counter, bool _isHexLiteral)
{
	solAssert(
