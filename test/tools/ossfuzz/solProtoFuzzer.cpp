...
evmc::result
executeContract(EVMHost& _hostContext, bytes const& _functionHash, evmc_address _deployedAddress)
{
...
)
{
	try
	{
		...
		return std::make_pair(
			solCompilationFramework
				.compileContract(_sourceCode, _contractName, _libraryAddresses, _optimization),
...
evmc::result deployAndExecute(EVMHost& _hostContext, bytes _byteCode, std::string _hexEncodedInput)
{
	...
	evmc::result callResult =
		executeContract(_hostContext, fromHex(_hexEncodedInput), createResult.create_address);
...
)
{
	...
	{
		tie(libraryBytecode, libIds) =
			compileContract(_sourceCode, _libraryName, {}, _optimization);
		// Deploy contract and signal failure if deploy failed
	...
	auto [bytecode, ids] =
		compileContract(_sourceCode, _contractName, _libraryAddressMap, _optimization);

	return deployAndExecute(hostContext, bytecode, ids[_methodName].asString());
}
}
...
DEFINE_PROTO_FUZZER(Program const& _input)
{
	...
	if (char const* dump_path = getenv("SOL_DEBUG_FILE"))
	{
		...
		sol_source = {std::istreambuf_iterator<char>(ifstr), std::istreambuf_iterator<char>()};
	...
	if (successState)
		solAssert(isOutputExpected(minimalResult), "Proto solc fuzzer: Output incorrect");
}
