	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			function <functionName>(addr, selector) -> combined {
				...
				functionName)("shl32", shiftLeftFunction(32))("shl64", shiftLeftFunction(64))
				.render();
		}
	);
}
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			function <functionName>(combined) -> addr, selector {
				...
				functionName)("shr32", shiftRightFunction(32))("shr64", shiftRightFunction(64))
				.render();
		}
	);
}
...
{
	string functionName = string(_assert ? "assert_helper" : "require_helper") +
		...
		[&]()
		{
			if (!_messageType)
				...
				function <functionName>(condition) {
					...
					_assert ? "invalid()" : "revert(0, 0)")("functionName", functionName)
					.render();

			int const hashHeaderSize = 4;
			int const byteSize = 8;
			u256 const errorHash = u256(FixedHash<hashHeaderSize>::Arith(
									   FixedHash<hashHeaderSize>(keccak256("Error(string)"))
								   ))
				<< (256 - hashHeaderSize * byteSize);

			string const encodeFunc =
				ABIFunctions(m_evmVersion, m_revertStrings, m_functionCollector)
					.tupleEncoder({_messageType}, {TypeProvider::stringMemory()});
			...
			function <functionName>(condition <messageVars>) {
				...
				functionName)("freeMemPointer", to_string(CompilerUtils::freeMemoryPointer))(
					   "errorHash",
					   formatNumber(errorHash)
			)("abiEncodeFunc", encodeFunc)("hashHeaderSize", to_string(hashHeaderSize))(
					   "messageVars",
		...
		[&]()
		{
			...
			case Type::Category::Integer:
			{
				...
				if (type.numBits() == 256)
					...
					templ(
						"body",
						"aligned := " + shiftLeftFunction(256 - type.numBits()) + "(value)"
					);
				break;
			}
			...
			case Type::Category::Contract:
				templ(
					"body",
					"aligned := " + leftAlignFunction(*TypeProvider::address()) + "(value)"
				);
				break;
			case Type::Category::Enum:
			{
				...
				templ(
					"body",
					"aligned := " + leftAlignFunction(IntegerType(8 * storageBytes)) + "(value)"
				);
				break;
			}
...
string YulUtilFunctions::shiftLeftFunction(size_t _numBits)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			)")("functionName", functionName)("numBits", to_string(_numBits))(
					   "hasShifts",
...
string YulUtilFunctions::shiftLeftFunctionDynamic()
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			)")("functionName", functionName)("hasShifts", m_evmVersion.hasBitwiseShifting())
				.render();
		}
	);
}
...
string YulUtilFunctions::typedShiftLeftFunction(Type const& _type, Type const& _amountType)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			)")("functionName", functionName)("cleanAmount", cleanupFunction(_amountType))(
					   "shift",
					   shiftLeftFunctionDynamic()
...
string YulUtilFunctions::typedShiftRightFunction(Type const& _type, Type const& _amountType)
{
	...
	string const functionName =
		"shift_right_" + _type.identifier() + "_" + _amountType.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]()
		{
			...
			function <functionName>(value, bits) -> result {
				...
				result := <cleanup>(<shift>(bits, <cleanup>(value)))
					   ...
					   "shift",
					   valueSigned ? shiftRightSignedFunctionDynamic() : shiftRightFunctionDynamic()
			)("cleanup", cleanupFunction(_type))
				.render();
		}
	);
...
string YulUtilFunctions::updateByteSliceFunction(size_t _numBytes, size_t _shiftBytes)
{
	...
	string functionName =
		"update_byte_slice_" + to_string(_numBytes) + "_shift_" + to_string(_shiftBytes);
	return m_functionCollector.createFunction(
		functionName,
		[&]()
		{
			...
			function <functionName>(value, toInsert) -> result {
				...
				result := or(value, and(toInsert, mask))
					   ...
					   "mask",
					   formatNumber(((bigint(1) << numBits) - 1) << shiftBits)
...
string YulUtilFunctions::updateByteSliceFunctionDynamic(size_t _numBytes)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			)")("functionName", functionName)("mask", formatNumber((bigint(1) << numBits) - 1))(
					   "shl",
					   shiftLeftFunctionDynamic()
			)
				.render();
		...
		[&]()
		{
			...
			function <functionName>(x, y) -> sum {
				...
				sum := add(x, y)
					   ...
					   "maxValue",
					   toCompactHexWithPrefix(u256(_type.maxValue()))
			)("minValue", toCompactHexWithPrefix(u256(_type.minValue())))(
					   "cleanupFunction",
					   cleanupFunction(_type)
			)
				.render();
		}
	);
		...
		[&]()
		{
			...
			function <functionName>(x, y) -> r {
				...
				r := <?signed>s</signed>div(x, y)
					   ...
					   "minVal",
					   toCompactHexWithPrefix(u256(_type.minValue()))
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			function <functionName>(x, y) -> r {
				...
				functionName)("signed", _type.isSigned())("cleanupFunction", cleanupFunction(_type))
				.render();
		}
	);
}
...
std::string YulUtilFunctions::resizeDynamicArrayFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			})")("functionName", functionName)("fetchLength", arrayLengthFunction(_type))(
					   "convertToSize",
					   arrayConvertLengthToSize(_type)
			)("dataPosition", arrayDataAreaFunction(_type))(
					   "clearStorageRange",
					   clearStorageRangeFunction(*_type.baseType())
...
string YulUtilFunctions::storageArrayPopFunction(ArrayType const& _type)
{
	...
	solUnimplementedAssert(
		...
		"Base type is not yet implemented."
	);
	if (_type.isByteArray())
		...
		[&]()
		{
			...
			function <functionName>(array) {
				...
				sstore(array, newLen)
					   ...
					   "indexAccess",
					   storageArrayIndexAccessFunction(_type)
		...
		[&]()
		{
			...
			function <functionName>(array) {
				...
				sstore(array, data)
				 ...
				 functionName)("extractByteArrayLength", extractByteArrayLengthFunction())(
					   "dataAreaFunction",
					   arrayDataAreaFunction(_type)
			)("indexAccess", storageArrayIndexAccessFunction(_type))(
					   "setToZero",
					   storageSetToZeroFunction(*_type.baseType())
			...
			function <functionName>(array, value) {
				...
				<!isByteArray>
					...
					<storeValue>(slot, offset, value)
					   ...
					   "extractByteArrayLength",
					   _type.isByteArray() ? extractByteArrayLengthFunction() : ""
			)("dataAreaFunction", arrayDataAreaFunction(_type))("isByteArray", _type.isByteArray())(
					   "indexAccess",
					   storageArrayIndexAccessFunction(_type)
			)("storeValue", updateStorageValueFunction(*_type.baseType()))(
					   "maxArrayLength",
					   (u256(1) << 64).str()
			)("shl", shiftLeftFunctionDynamic())("shr", shiftRightFunction(248))
				.render();
		}
	);
}
...
string YulUtilFunctions::clearStorageRangeFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("setToZero", storageSetToZeroFunction(_type))(
					   "increment",
					   _type.storageSize().str()
			)
				.render();
		}
	);
...
string YulUtilFunctions::clearStorageArrayFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("dynamic", _type.isDynamicallySized())(
					   "resizeArray",
					   _type.isDynamicallySized() ? resizeDynamicArrayFunction(_type) : ""
			)("clearRange",
			  clearStorageRangeFunction(
				  (_type.baseType()->storageBytes() < 32) ? *TypeProvider::uint256() :
															  *_type.baseType()
			  ))("lenToSize", arrayConvertLengthToSize(_type))("len", _type.length().str())
				.render();
		}
	);
}
		...
		[&]()
		{
			...
			case DataLocation::Storage:
			{
				...
				return Whiskers(R"(
					...
					})")("functionName", functionName)("multiSlot", baseType.storageSize() > 1)(
						   "itemsPerSlot",
						   to_string(32 / baseStorageBytes)
				...
				return Whiskers(R"(
					function <functionName>(length) -> size {
						...
						<!byteArray>
						   ...
						   "stride",
						   to_string(
							   _type.location() == DataLocation::Memory ? _type.memoryStride() :
																			_type.calldataStride()
						   )
...
string YulUtilFunctions::storageArrayIndexAccessFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("arrayLen", arrayLengthFunction(_type))(
					   "dataAreaFunc",
					   arrayDataAreaFunction(_type)
			)("multipleItemsPerSlot",
			  _type.baseType()->storageBytes() <= 16)("isBytesArray", _type.isByteArray())(
					   "storageSize",
					   _type.baseType()->storageSize().str()
...
string YulUtilFunctions::calldataArrayIndexAccessFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("stride", to_string(_type.calldataStride()))(
					   "dynamicallySized",
					   _type.isDynamicallySized()
			)("dynamicallyEncodedBase", _type.baseType()->isDynamicallyEncoded())(
					   "dynamicallySizedBase",
					   _type.baseType()->isDynamicallySized()
			)("arrayLen", toCompactHexWithPrefix(_type.length()))(
					   "accessCalldataTail",
					   _type.baseType()->isDynamicallyEncoded() ?
							 accessCalldataTailFunction(*_type.baseType()) :
							 ""
			)
				.render();
		}
		...
		[&]()
		{
			...
			function <functionName>(offset, length, startIndex, endIndex) -> offsetOut, lengthOut {
				...
				lengthOut := sub(endIndex, startIndex)
					   ...
					   "revertSliceStartAfterEnd",
					   revertReasonIfDebug("Slice starts after end")
...
string YulUtilFunctions::accessCalldataTailFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("dynamicallySized", _type.isDynamicallySized())(
					   "neededLength",
					   toCompactHexWithPrefix(_type.calldataEncodedTailSize())
			)("calldataStride",
			  toCompactHexWithPrefix(
				  _type.isDynamicallySized() ?
						dynamic_cast<ArrayType const&>(_type).calldataStride() :
						0
			  ))("invalidCalldataTailOffset", revertReasonIfDebug("Invalid calldata tail offset"))(
					   "invalidCalldataTailLength",
					   revertReasonIfDebug("Invalid calldata tail length")
...
string
YulUtilFunctions::mappingIndexAccessFunction(MappingType const& _mappingType, Type const& _keyType)
{
	solAssert(_keyType.sizeOnStack() <= 1, "");

	string functionName =
		"mapping_index_access_" + _mappingType.identifier() + "_of_" + _keyType.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]()
		{
			...
			)")("functionName", functionName)("key", _keyType.sizeOnStack() > 0 ? "key" : "")(
						   "hash",
						   packedHashFunction(
			...
			else
			{
				...
				if (_keyType.sizeOnStack() == 0)
					templ(
						"convertedKey",
						conversionFunction(_keyType, *_mappingType.keyType()) + "()"
					);
				else
					templ(
						"convertedKey",
						conversionFunction(_keyType, *_mappingType.keyType()) + "(key)"
					);
				return templ.render();
			}
		}
...
string
YulUtilFunctions::readFromStorage(Type const& _type, size_t _offset, bool _splitFunctionTypes)
{
	if (_type.category() == Type::Category::Function)
		solUnimplementedAssert(!_splitFunctionTypes, "");
	string functionName = "read_from_storage_" + string(_splitFunctionTypes ? "split_" : "") +
		"offset_" + to_string(_offset) + "_" + _type.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]
		{
		...
		)")("functionName", functionName)("extract", extractFromStorageValue(_type, _offset, false))
				.render();
		}
	);
}
...
string YulUtilFunctions::readFromStorageDynamic(Type const& _type, bool _splitFunctionTypes)
{
	if (_type.category() == Type::Category::Function)
		solUnimplementedAssert(!_splitFunctionTypes, "");
	string functionName = "read_from_storage_dynamic" +
		string(_splitFunctionTypes ? "split_" : "") + "_" + _type.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]
		{
			...
			function <functionName>(slot, offset) -> value {
				...
				functionName)("extract", extractFromStorageValueDynamic(_type, _splitFunctionTypes))
				.render();
		}
	);
}
...
{
	string const functionName = "update_storage_value_" +
		(_offset.has_value() ? ("offset_" + to_string(*_offset)) : "") + _type.identifier();
		...
		[&]
		{
			if (_type.isValueType())
			{
				...
				function <functionName>(slot, <offset>value) {
				  ...
				  _offset.has_value() ? "" : "offset, ")("prepare", prepareStoreFunction(_type))
					.render();
			}
			else
			{
...
{
	string const functionName = string("write_to_memory_") + _type.identifier();
	...
	return m_functionCollector.createFunction(
		...
		[&]
		{
			...
			)")("functionName", functionName)("combine", combineExternalFunctionIdFunction())
					.render();
			}
			...
			)")("functionName", functionName)("cleanup", cleanupFunction(_type))
					.render();
			}
			else  // Should never happen
			{
				solAssert(false, "Memory store of type " + _type.toString(true) + " not allowed.");
			}
		}
	);
}
...
string YulUtilFunctions::extractFromStorageValueDynamic(Type const& _type, bool _splitFunctionTypes)
{
	...
	string functionName = "extract_from_storage_value_dynamic" +
		string(_splitFunctionTypes ? "split_" : "") + _type.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]
		{
		...
		)")("functionName", functionName)("shr", shiftRightFunctionDynamic())(
					   "cleanupStorage",
					   cleanupFromStorageFunction(_type, _splitFunctionTypes)
			)
				.render();
		}
	);
...
string YulUtilFunctions::extractFromStorageValue(
	Type const& _type,
	size_t _offset,
...
)
{
	...
	string functionName = "extract_from_storage_value_" +
		string(_splitFunctionTypes ? "split_" : "") + "offset_" + to_string(_offset) +
		_type.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]
		...
		)")("functionName", functionName)("shr", shiftRightFunction(_offset * 8))(
					   "cleanupStorage",
					   cleanupFromStorageFunction(_type, _splitFunctionTypes)
			)
				.render();
...
string YulUtilFunctions::cleanupFromStorageFunction(Type const& _type, bool _splitFunctionTypes)
{
	...
	string functionName = string("cleanup_from_storage_") + (_splitFunctionTypes ? "split_" : "") +
		_type.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]
		{
			...
			if (storageBytes == 32)
				...
				templ(
					"cleaned",
					"and(value, " + toCompactHexWithPrefix((u256(1) << (8 * storageBytes)) - 1) +
						")"
				);
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			if (_type.category() == Type::Category::FixedBytes)
				templ(
					"actualPrepare",
					shiftRightFunction(256 - 8 * _type.storageBytes()) + "(value)"
				);
			else
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			function <functionName>(size) -> memPtr {
				...
				to_string(CompilerUtils::freeMemoryPointer))("functionName", functionName)
				.render();
		}
	);
}
...
string YulUtilFunctions::zeroComplexMemoryArrayFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("stride", to_string(_type.memoryStride()))(
					   "zeroValue",
					   zeroValueFunction(*_type.baseType(), false)
			)
				.render();
		}
	);
...
string YulUtilFunctions::allocateMemoryArrayFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			)")("functionName", functionName)("alloc", allocationFunction())(
					   "allocSize",
					   arrayAllocationSizeFunction(_type)
...
string YulUtilFunctions::allocateAndInitializeMemoryArrayFunction(ArrayType const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			)")("functionName", functionName)("allocArray", allocateMemoryArrayFunction(_type))(
					   "allocSize",
					   arrayAllocationSizeFunction(_type)
			)("zeroArrayFunction",
			  zeroMemoryArrayFunction(_type))("dynamic", _type.isDynamicallySized())
				.render();
		}
	);
}
...
string YulUtilFunctions::conversionFunction(Type const& _from, Type const& _to)
{
	if (_from.category() == Type::Category::Function)
	{
		...
		solAssert(
			fromType.isImplicitlyConvertibleTo(targetType) &&
				...
				(fromType.kind() == FunctionType::Kind::Internal ||
				 fromType.kind() == FunctionType::Kind::External) &&
				fromType.kind() == targetType.kind(),
			"Invalid function type conversion requested."
		);
		string const functionName = "convert_" + _from.identifier() + "_to_" + _to.identifier();
		return m_functionCollector.createFunction(
			functionName,
			[&]()
			{
	...
	string functionName = "convert_" + _from.identifier() + "_to_" + _to.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]()
		{
			...
			case Type::Category::Contract:
			{
				...
				else if (toCategory == Type::Category::Enum)
				{
					...
					body = Whiskers(
							   "converted := <cleanEnum>(<cleanInt>(value))"
				...
				else
				{
					...
					{
						IntegerType const& from = fromCategory == Type::Category::Integer ?
			...
			case Type::Category::Bool:
			{
				...
				body = Whiskers("converted := <clean>(value)")("clean", cleanupFunction(_from))
						   .render();
				break;
			}
			case Type::Category::FixedPoint:
			...
			case Type::Category::Array:
			{
				...
				else
				{
					...
					case DataLocation::Storage:
						...
						break;
					case DataLocation::Memory:
						// Copy the array to a free position in memory, unless it is already
						// in memory.
						solUnimplementedAssert(
			...
			case Type::Category::FixedBytes:
			{
				...
				if (toCategory == Type::Category::Integer)
					body = Whiskers("converted := <convert>(<shift>(value))")(
							   "shift",
							   shiftRightFunction(256 - from.numBytes() * 8)
				...
				else
				{
					...
					solAssert(
						toCategory == Type::Category::FixedBytes,
						"Invalid type conversion requested."
					);
					body = Whiskers("converted := <clean>(value)")("clean", cleanupFunction(from))
							   .render();
				}
				break;
			}
			...
			case Type::Category::Enum:
			{
				...
				body = Whiskers("converted := <clean>(value)")("clean", cleanupFunction(enumType))
						   .render();
				break;
			}
		...
		[&]()
		{
			...
			case Type::Category::Integer:
			{
				...
				if (type.numBits() == 256)
					...
					templ(
						"body",
						"cleaned := signextend(" + to_string(type.numBits() / 8 - 1) + ", value)"
					);
				else
					templ(
						"body",
			...
			case Type::Category::Contract:
			{
				AddressType addressType(
					dynamic_cast<ContractType const&>(_type).isPayable() ?
			...
			case Type::Category::Enum:
			{
				// Out of range enums cannot be truncated unambigiously and therefore it should
				// be an error.
...
{
	string functionName =
		string("validator_") + (_revertOnFailure ? "revert_" : "assert_") + _type.identifier();
	return m_functionCollector.createFunction(
		functionName,
		[&]()
		{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			...
			templ(
				"packedEncode",
				ABIFunctions(m_evmVersion, m_revertStrings, m_functionCollector)
					.tupleEncoderPacked(_givenTypes, _targetTypes)
			);
			return templ.render();
		}
	);
...
std::string YulUtilFunctions::decrementCheckedFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("minval", toCompactHexWithPrefix(minintval))(
					   "lt",
					   type.isSigned() ? "slt" : "lt"
			)("cleanupFunction", cleanupFunction(_type))
				.render();
		}
	);
...
std::string YulUtilFunctions::incrementCheckedFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("maxval", toCompactHexWithPrefix(maxintval))(
					   "gt",
					   type.isSigned() ? "sgt" : "gt"
			)("cleanupFunction", cleanupFunction(_type))
				.render();
		}
	);
...
string YulUtilFunctions::zeroValueFunction(Type const& _type, bool _splitFunctionTypes)
{
	...
	string const functionName =
		"zero_value_for_" + string(_splitFunctionTypes ? "split_" : "") + _type.identifier();
			...
			if (_type.isValueType())
			{
				solAssert(
					(_type.hasSimpleZeroValueInMemory() ||
					 (fType &&
					  (fType->kind() == FunctionType::Kind::Internal ||
					   fType->kind() == FunctionType::Kind::External))),
					""
				);
				templ("zeroValue", "0");
			}
			else
			{
				...
				if (auto const* arrayType = dynamic_cast<ArrayType const*>(&_type))
				{
					if (_type.isDynamicallySized())
						...
						templ(
							"zeroValue",
							allocateAndInitializeMemoryArrayFunction(*arrayType) + "(" +
								to_string(unsigned(arrayType->length())) + ")"
						);
				}
				else if (auto const* structType = dynamic_cast<StructType const*>(&_type))
					templ(
						"zeroValue",
						allocateAndInitializeMemoryStructFunction(*structType) + "()"
					);
				else
					solUnimplementedAssert(false, "");
			}
...
string YulUtilFunctions::storageSetToZeroFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			if (_type.isValueType())
			...
			)")("functionName", functionName)("store", updateStorageValueFunction(_type))(
						   "zeroValue",
						   zeroValueFunction(_type)
				)
					.render();
				...
				function <functionName>(slot, offset) {
					<clearArray>(slot)
						   ...
						   "clearArray",
						   clearStorageArrayFunction(dynamic_cast<ArrayType const&>(_type))
				)
					.render();
			else
				solUnimplemented(
					"setToZero for type " + _type.identifier() + " not yet implemented!"
				);
		}
	);
}
...
string YulUtilFunctions::conversionFunctionSpecial(Type const& _from, Type const& _to)
{
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			if (auto fromTuple = dynamic_cast<TupleType const*>(&_from),
				toTuple = dynamic_cast<TupleType const*>(&_to);
				fromTuple && toTuple &&
				fromTuple->components().size() == toTuple->components().size())
			{
				...
				for (size_t i = 0; i < fromTuple->components().size(); ++i)
				{
					...
					if (toComponent)
					{
						conversions += suffixedVariableNameList(
										   "converted",
										   destStackSize,
				...
				function <functionName>(<values>) <arrow> <converted> {
					<conversions>
						   ...
						   "values",
						   suffixedVariableNameList("value", 0, sourceStackSize)
				)("arrow", destStackSize > 0 ? "->" : "")(
						   "converted",
						   suffixedVariableNameList("converted", 0, destStackSize)
				)("conversions", conversions)
					.render();
			}

			if (_from.category() == Type::Category::Array &&
				_to.category() == Type::Category::Array)
			{
				...
				solUnimplementedAssert(
					fromArrayType.isByteArray() && toArrayType.isByteArray(),
					""
				);
				...
				templ(
					"copyToMemory",
					copyToMemoryFunction(fromArrayType.location() == DataLocation::CallData)
				);
				return templ.render();
			}
			...
			solUnimplementedAssert(
				...
				"Type conversion " + _from.toString() + " -> " + _to.toString() +
					" not yet implemented."
			);
			...
			if (_to.category() == Type::Category::FixedBytes)
			{
				...
				templ(
					"data",
					formatNumber(
						h256::Arith(h256(data, h256::AlignLeft)) & (~(u256(-1) >> (8 * numBytes)))
					)
				);
				return templ.render();
			}
...
{
	string functionName =
		string("read_from_") + (_fromCalldata ? "calldata" : "memory") + _type.identifier();
	...
	return m_functionCollector.createFunction(
		...
		[&]
		{
			...
			if (auto const* funType = dynamic_cast<FunctionType const*>(&_type))
				...
				)")("functionName", functionName)("load", _fromCalldata ? "calldataload" : "mload")(
							   "splitFunction",
							   splitExternalFunctionIdFunction()
					)
						.render();
			...
			function <functionName>(ptr) -> value {
				...
				functionName)("fromCalldata", _fromCalldata)("validate", validatorFunction(_type))
				// Byte array elements generally need cleanup.
				// Other types are cleaned as well to account for dirty memory e.g. due to inline
				// assembly.
				("cleanup", cleanupFunction(_type))
					.render();
		}
...
string YulUtilFunctions::tryDecodeErrorMessageFunction()
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("shr224", shiftRightFunction(224))(
					   "ErrorSignature",
					   FixedHash<4>(util::keccak256("Error(string)")).hex()
			)("freeMemoryPointer",
			  to_string(CompilerUtils::freeMemoryPointer))("roundUp", roundUpFunction())
				.render();
		}
	);
}
...
string YulUtilFunctions::extractReturndataFunction()
{
	...
	return m_functionCollector.createFunction(
		...
		)")("functionName", functionName)("supportsReturndata", m_evmVersion.supportsReturndata())(
					   "allocate",
					   allocationFunction()
			)("roundUp",
			  roundUpFunction())("emptyArray", zeroValueFunction(*TypeProvider::bytesMemory()))
				.render();
		}
	);
}
...
)
{
	string functionName = "copy_arguments_for_constructor_" +
		toString(_contract.constructor()->id()) + "_object_" + _contract.name() + "_" +
	...
	return m_functionCollector.createFunction(
		...
		[&]()
		{
			string returnParams = suffixedVariableNameList(
				"ret_param_",
				0,
				_contract.constructor()->parameters().size()
			);
		...
		)")("functionName", functionName)("retParams", returnParams)("object", _creationObjectName)(
					   "allocate",
					   allocationFunction()
			)("abiDecode",
			  abiFunctions
				  .tupleDecoder(FunctionType(*_contract.constructor()).parameterTypes(), true))
				.render();
		}
	);
}
