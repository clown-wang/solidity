...
variant<unique_ptr<Block>, ErrorList>
Program::parseObject(Dialect const& _dialect, CharStream _source)
{
	ErrorList errors;
...
variant<unique_ptr<AsmAnalysisInfo>, ErrorList>
Program::analyzeAST(Dialect const& _dialect, Block const& _ast)
{
	ErrorList errors;
	ErrorReporter errorReporter(errors);
