...
SourceReferenceExtractor::Message
SourceReferenceExtractor::extract(util::Exception const& _exception, string _category)
{
...
SourceReference
SourceReferenceExtractor::extract(SourceLocation const* _location, std::string message)
{
	...
	int locationLength =
		isMultiline ? int(line.length()) - start.column : end.column - start.column;

	if (locationLength > 150)
	{
		auto const lhs = static_cast<size_t>(start.column) + 35;
		string::size_type const rhs =
	...
	if (line.length() > 150)
	{
		...
		line = line.substr(
			...
			static_cast<size_t>(min(start.column, 35)) +
				static_cast<size_t>(min(locationLength + 35, len - start.column))
		);
