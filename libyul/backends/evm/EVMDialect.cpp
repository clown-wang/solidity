...
createEVMFunction(string const& _name, evmasm::Instruction _instruction)
{
	...
	f.controlFlowSideEffects.terminates =
		evmasm::SemanticInformation::terminatesControlFlow(_instruction);
...
map<YulString, BuiltinFunctionForEVM>
createBuiltins(langutil::EVMVersion _evmVersion, bool _objectAccess)
{
	...
	for (auto const& instr: evmasm::c_instructions)
	{
		...
		if (!evmasm::isDupInstruction(opcode) && !evmasm::isSwapInstruction(opcode) &&
			!evmasm::isPushInstruction(opcode) && opcode != evmasm::Instruction::JUMP &&
			opcode != evmasm::Instruction::JUMPI && opcode != evmasm::Instruction::JUMPDEST &&
	...
	if (_objectAccess)
	{
		...
		builtins.emplace(createFunction(
			   ...
			   std::function<void(Expression const&)> const&)
			{
				...
				{
					vector<size_t> subIdPath = _context.subIDs.count(dataName) == 0 ?
	...
	EVMDialect(_evmVersion, _objectAccess)
{
	...
	m_functions.insert(createFunction(
		"bool_to_u256",
		1,
		1,
		{},
